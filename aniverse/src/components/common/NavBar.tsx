      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-black/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <Link href="/anime/demon-slayer" className="text-sm text-white/70 hover:text-white transition-colors">Anime</Link>
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white/50 hover:text-white text-sm px-4 py-2 rounded-full transition-all"
              >
                <Search size={14} />
                <span className="text-xs">Search...</span>
                <kbd className="text-xs text-white/20 border border-white/10 rounded px-1.5 py-0.5">⌘K</kbd>
              </button>
              <button className="p-2 text-white/70 hover:text-white transition-colors">
                <Bell size={18} />
              </button>
              <button style={{ background: 'linear-gradient(135deg, #FF6B9D, #FF8C42)' }}
                className="flex items-center gap-2 text-white text-sm font-bold px-4 py-2 rounded-full hover:opacity-90 transition-opacity">
                <User size={16} />
                <Link href="/login"
        ...
        {isOpen && (
          <div className="md:hidden border-t border-white/10 bg-black/95 px-4 py-4 flex flex-col gap-4">
            ...
            <Link href="/anime/demon-slayer" className="text-sm text-white/70 hover:text-white" onClick={() => setIsOpen(false)}>Anime</Link>
            ...
            <button style={{ background: 'linear-gradient(135deg, #FF6B9D, #FF8C42)' }}
              className="flex items-center justify-center gap-2 text-white text-sm font-bold px-4 py-2 rounded-full">
              <User size={16} />
              Sign In
            </button>
          </div>
